...
function initMenu() {
    ...
    && !( browser.isIE && browser.versionMajor < 5 ) ) {
		...
		// Correction of the first panel position to view
		...
		for (i = 0; i < first_children.length; i++) {
			...
			if (ul != null) {
				ul.style.marginTop  = 25 +"px";
			 	ul.style.marginLeft  = - first_children[i].offsetWidth +"px";
			}
		}
        ...
        for ( var i=0; i<lis.length; i++ ) {
            ...
            if ( uls.length > 0 ) {
				...
				for (j = 0; j < first_ul_children.length; j++) {
					first_ul_children[j].getElementsByTagName('input').item(0).style.width 
						= uls.item(0).offsetWidth -2 +"px";
				}
...
function hideUlUnder( id ) {
	document.getElementById(id).getElementsByTagName('input')[0].className="";/*m*/
...
function show() {     // show the sub menu
    ...
    while(currentNode) {
            if( currentNode.nodeName=='LI') {
                currentNode.getElementsByTagName('input')[0].className = 'linkOver';
            }
...
function hideAllOthersUls( currentLi ) {
	...
	//currentLi.getElementsByTagName('input')[0].className="";
...
function hideUlUnderLi( li ) {
    var as = li.getElementsByTagName('input');

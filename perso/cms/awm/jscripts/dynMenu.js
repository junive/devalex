...
function initMenu() {
    ...
    && !( browser.isIE && browser.versionMajor < 5 ) ) {
		...
		set_menu_margin(menu);
		
    }
}

function set_menu_margin(elmt) {
	var first_children =  getDirectChildren(elmt);
	for (i = 0; i < first_children.length; i++) {
		ul =  first_children[i].getElementsByTagName('ul').item(0);
		if (ul != null) { // First cild panel
			var ie_margin = (browser.isIE) ? 4 : 0;
			ul.style.marginTop  = ie_margin + 24 +"px";
			ul.style.marginLeft  = - first_children[i].offsetWidth + 1 +"px";
			var second_children = ul.getElementsByTagName('ul');
			for (j = 0; j < second_children.length; j++) { // The fllowing panel child
				uls =  second_children[j];
				if (uls != null) {
					var ie_margin = (browser.isIE) ? 1 : 0;
					uls.style.marginTop  = ie_margin - 28 + "px";
					uls.style.marginLeft  = getParent(uls, "li").offsetWidth + 1 +"px";
				}
			}
		}
	}
...
// Resize a column of menu, if not so, the <a> took his own size
function set_menu_size(elmt) {
	first_ul_children = getDirectChildren(elmt);
	for (j = 0; j < first_ul_children.length; j++) {
		my_a = first_ul_children[j].getElementsByTagName('a').item(0);
		my_a.style.width = elmt.offsetWidth - 12 +"px";
	}
}

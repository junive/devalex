...
Asset.prototype = {
	initialize: function(codec, urls) {
		...
		this.urls = urls;
		this.isRadio = ('undefined' != arguments[2]) ? arguments[2] : null;
		this.current = 0;
	},
...
Casing.prototype = Object.extend(new AbstractPlugin(), {
  initialize: function(eventHandler) {
    ...
    new PeriodicalExecuter(
      this.pstatus_poller.bindAsEventListener(this),
      audiolib_latency);
  ...
  attach_plugins: function() {
	if ('Linux' == this.OS) {
		this.eventHandler('notSupportedProduct', 'Linux', '');
		//report_missing_plugin('linux', this.eventHandler);
		this.rp = this.wmqtp = this.current_player = null;
	} else { 
		if ('Mac' == this.OS && ('Safari' == this.browser || 'Firefox' == this.browser) && this.plugins.QTDetected) {
			this.wmqtp = new QTP();
			if ('Firefox' == this.browser) {
				this.useTimeout = false;
			}
		} else if (this.plugins.WMDetected) {
			if ('Firefox' == this.browser &&
				'Windows' == this.OS &&
				!this.plugins.WMPFirefoxDetected) {
				this.wmqtp = null;
	...
	if ('undefined' != typeof codec) {
  	...
  	switch (codec) {
  		...
  		case 'asf':
  			if (null != this.wmqtp) {
  				this.current_player = this.wmqtp;
  				this.current_player.isRadio = this.asset.isRadio;
  				this.current_player.setUrl(url);
  			} else
  		...
  		default:
  			if (null != this.wmqtp) {
				...
				if (this.eventHandler)
					this.eventHandler('pluginStatus', __('UnsupportedCodec'), null);
			}
  	}
  },
  ...
  pstatus_poller: function() {
    pstatus = this.getPluginStatus();
 	...
 	if (pstatus == 'Connecting') document.getElementById("playerProgressBarLeft").style.width='100px';
  	else if (pstatus == 'Buffering') document.getElementById("playerProgressBarLeft").style.width='200px';
  	else if (pstatus == 'Playing') document.getElementById("playerProgressBarLeft").style.width='314px';
  	else if (pstatus == 'Stopped') document.getElementById("playerProgressBarLeft").style.width='1px';
 	// report pstatus changes to the client's listener
  	if (this.eventHandler && pstatus != this.previous_pstatus)
  	this.eventHandler('pluginStatus', __(pstatus), null);
    	this.previous_pstatus = pstatus;
  },
  
  use_next_url: function() {
	  if(this.asset && this.asset.has_url()) {
		Url = this.asset.next_url();
	    //spy('use next url: ' + Url);
  ...
  asset_watchdog: function(pstatus) {
    ...
    if(this.timeout[0] && this.get_timestamp() - this.timeout[0] > audiolib_timeout*1000) {
      ...
      this.stop_timeout(0);
      if (this.eventHandler)
  		  this.eventHandler('pluginStatus', __('TimeOut'), null);
    ...
    if (this.timeout[1] && this.timeLimit && this.get_timestamp() - this.timeout[1] > this.timeLimit * 1000) {
		...
		this.stop_timeout(1);
		if (this.eventHandler)
			this.eventHandler('timeLimit', __('TimeLimit'), null);
    }
	...
	if (('stopped' == pstatus.toLowerCase() && !this.buttonStop) || 'end of stream' == pstatus.toLowerCase()) {
  	  if (this.use_next_url())
  	    this.play();
	}
...
QTP.prototype = Object.extend(new AbstractPlugin(), {
	initialize: function() {
		...
		//Url = this.preparePlaylistUrl(Url);
	...
	preparePlaylistUrl: function(Url) {
		protocle = Url.split('://');
		Url = (protocle.length > 1) ? protocle[1] : protocle[0];
		separator =  (Url.substring(Url.length - 1) == '/') ? '' : '/';
		return 'http://wholeworldjob.com/playlistredirect/' + Url + separator + 'list.pls';
	},

	QTUrl: function(Url) {
		return 'http://wholeworldjob.com/qtredirect/' + Url.replace('://', '/') + '/trick.mov';
	},
	
	setUrl: function(Url) {
		Url = (this.isRadio) ? this.preparePlaylistUrl(Url) : this.QTUrl(Url);
		try {
			this.qtp.SetURL(Url)
		} catch (err) {
			this.initialize(Url);
	...
	getPluginStatus: function() {
		...
		if ('Complete' == playState /*&& this.emulatedStatus != 'Error'*/) {
			if (this.qtp.GetEndTime() == this.qtp.GetTime()) {
				playState = 'End of stream';
			} else {
				document.getElementById("playerProgressBarLeft").style.width='310px';
			...
			if (playState.indexOf('Error') != -1) {
				...
				if ('Waiting' == playState) {
					...
					document.getElementById("playerProgressBarLeft").style.width='100px';
				} else {
					playState = 'Buffering';
					document.getElementById("playerProgressBarLeft").style.width='200px';
				}
			}
		}
		} catch(err) {
...
RPL.prototype = Object.extend(new AbstractPlugin(), {
	...
	play: function() {
		...
		if(this.rpl/* && this.rpl.DoPlay*/)
		  this.rpl.DoPlay();
	},
	
	stop: function() {
		AudioLib.AudioPlayer.rp.clearRealInterval();
	  if(this.rpl /*&& this.rpl.DoStop*/)
  		this.rpl.DoStop();
	},
	
	pause: function() {
	  if(this.rpl /*&& this.rpl.DoPause*/)
  		this.rpl.DoPause();
	},
	...
	setVolume: function (vol) {
		...
		if(this.rpl != null && 'undefined' != typeof this.rpl  && 'function' == typeof this.rpl.SetVolume) {
  			this.rpl.SetVolume(volumo);
  		}
	},	
...
//				audio_player.play();
  	...
  	else if (pstatus == 'Playing') document.getElementById("playerProgressBarLeft").style.width='300px';
  	else if (pstatus == 'Stopped') document.getElementById("playerProgressBarLeft").style.width='1px';
  	*/
  	if ('' != error) {
...
function __(key) {
 var translation = $H({
	...
	if (translation[AudioLib.AudioPlayer.language].keys().indexOf(key) != -1) {
		return translation[AudioLib.AudioPlayer.language][key];
	} else {
		return key;
	}
}
